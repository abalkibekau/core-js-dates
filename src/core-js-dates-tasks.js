...
function toTwoDigitsString(number) {
...
function getTime(date) {
  return `${toTwoDigitsString(date.getHours())}:${toTwoDigitsString(date.getMinutes())}:${toTwoDigitsString(date.getSeconds())}`;
}
...
function getNextFriday(date) {
  ...
  const shift = day > 4 ? 12 - day : 5 - day;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.floor((new Date(dateEnd) - new Date(dateStart)) / 86400000) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return dateToCheck - start >= 0 && end - dateToCheck >= 0;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.floor((date - startOfYear) / 86400000 / 7) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  return nextFriday.getDate() === 13
    ? nextFriday
    : getNextFridayThe13th(nextFriday);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (date <= endDate) {
    if (workDays > 0) {
      schedule.push(
        `${toTwoDigitsString(date.getDate())}-${toTwoDigitsString(date.getMonth() + 1)}-${date.getFullYear()}`
      );
      workDays -= 1;
